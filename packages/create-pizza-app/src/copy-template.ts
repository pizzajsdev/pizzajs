...
function isLocalFilePath(input: string): boolean {
  try {
    return (
      input.startsWith('file://') || fs.existsSync(path.isAbsolute(input) ? input : path.resolve(process.cwd(), input))
    )
  } catch (_) {
    return false
  }
...
async function copyTemplateFromGithubRepoUrl(repoUrl: string, destPath: string, options: CopyTemplateOptions) {
  await downloadAndExtractRepoTarball(getRepoInfo(repoUrl), destPath, options)
}

async function copyTemplateFromGenericUrl(url: string, destPath: string, options: CopyTemplateOptions) {
  await copyTemplateFromRemoteTarball(url, destPath, options)
}

async function copyTemplateFromLocalFilePath(filePath: string, destPath: string): Promise<boolean> {
...
async function extractLocalTarball(tarballPath: string, destPath: string): Promise<void> {
  try {
...
async function downloadAndExtractRepoTarball(repo: RepoInfo, destPath: string, options: TarballDownloadOptions) {
...
async function downloadAndExtractTarball(
  ...
  if (!response.body || response.status !== 200) {
    if (token) {
      throw new CopyTemplateError(
        `There was a problem fetching the file${isGithubUrl ? ' from GitHub' : ''}. The request ` +
      ...
      tar.extract(downloadPath, {
        map(header) {
          ...
          if (filePath) {
            ...
            if (
              (filePath.endsWith(path.posix.sep) && header.name.startsWith(filePath)) ||
              (!filePath.endsWith(path.posix.sep) && header.name.startsWith(filePath + path.posix.sep))
            ) {
...
async function writeReadableStreamToWritable(stream: ReadableStream, writable: stream.Writable) {
...
function isValidGithubRepoUrl(input: string | URL): input is URL | GithubUrlString {
  if (!isUrl(input)) {
    return false
  }
  try {
...
function isGithubReleaseAssetUrl(url: string) {
  ...
  return (
    ...
    (url.includes('/releases/download/') || url.includes('/releases/latest/download/'))
  )
}
...
type GithubUrlString = `https://github.com/${string}/${string}` | `https://www.github.com/${string}/${string}`
