...
async function getContext(argv: string[]): Promise<Context> {
  ...
  if (selectedReactRouterVersion) {
    if (semver.valid(selectedReactRouterVersion)) {
      ...
      selectedReactRouterVersion = semver.coerce(selectedReactRouterVersion)!.version
    } else {
      log(
        `\n${color.warning(
  ...
  let context: Context = {
    tempDir: path.join(
      ...
      `create-pizza-app--${Math.random().toString(36).substr(2, 8)}`,
    ),
    cwd,
    overwrite,
    interactive,
...
async function introStep(ctx: Context) {
  log(`\n${' '.repeat(9)}${color.green(color.bold('create-pizza-app'))} ${color.bold(`v${ctx.reactRouterVersion}`)}`)
...
async function projectNameStep(ctx: Context) {
  ...
  if (ctx.cwd) {
    await sleep(100)
    info('Directory:', ['Using ', color.reset(ctx.cwd), ' as project directory'])
  }
...
async function copyTempDirToAppDirStep(ctx: Context) {
  ...
  if (collisions.length > 0) {
    ...
    if (ctx.overwrite) {
      info('Overwrite:', `overwriting files due to \`--overwrite\`:${getFileList('           ')}`)
...
async function gitInitStep(ctx: Context) {
  ...
  await loadingIndicator({
    ...
    while: async () => {
      ...
      let commitMsg = 'Initial commit from create-pizza-app'
      try {
...
async function updatePackageJSON(ctx: Context) {
  ...
  fs.promises.writeFile(packageJSONPath, JSON.stringify(sortPackageJSON(packageJSON), null, 2), 'utf-8')
}
...
function printHelp(ctx: Context) {
  // prettier-ignore
  let output = `
${title("create-pizza-app")}

${color.heading("Usage")}:

${color.dim("$")} ${color.greenBright("create-pizza-app")} ${color.arg("<projectDir>")} ${color.arg("<...options>")}

${color.heading("Values")}:

${color.arg("projectDir")}          ${color.dim(`The PizzaJS project directory`)}
...
${[
  ...
  return `${str}\n${color.dim("$")} ${color.greenBright("create-pizza-app")} my-app ${color.arg(`--template ${example}`)}`;
}, "")}
